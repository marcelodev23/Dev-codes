...
@pytest.mark.parametrize(
    "nome, saudacao_esperada", [("Alice", "Olá, Alice!"), ("Bob", "Olá, Bob!")]
)
...
@pytest.mark.parametrize(
    "nome, idioma, saudacao_esperada",
    [
        ("Alice", "inglês", "Hello, Alice!"),
        ("Bob", "espanhol", "Hola, Bob!"),
        ("Charlie", "francês", "Bonjour, Charlie!"),
    ],
)
...
@pytest.mark.parametrize(
    "base, expoente, resultado_esperado", [(2, 3, 8), (5, 2, 25), (3, 0, 1)]
)
...
@pytest.mark.parametrize(
    "idade, resultado_esperado", [(25, True), (0, True), (200, False)]
)
...
@pytest.mark.parametrize(
    ...
    [("exemplo@email.com", True), ("invalido@.com", False), ("sem_arroba.com", False)],
)
...
@pytest.mark.parametrize(
    "horas_trabalhadas, taxa_hora, resultado_esperado",
    [(40, 10, 400), (0, 15, 0), (45, 12.5, 562.5)],
)
def test_calcular_pagamento(horas_trabalhadas, taxa_hora, resultado_esperado):
    assert (
        calcular_pagamento(horas_trabalhadas=horas_trabalhadas, taxa_hora=taxa_hora)
        == resultado_esperado
    )
...
@pytest.mark.parametrize(
    "a, b, c, resultado_esperado", [(5, 2, 8, 8), (-10, -20, -5, -5), (3, 3, 3, 3)]
)
...
@pytest.mark.parametrize(
    ...
    [("Hello World", (2, 8)), ("AbCDeF", (4, 2)), ("12345", (0, 0))],
)
...
@pytest.mark.parametrize(
    "iteravel, resultado_esperado", [([1, 2, 3], 3), ("Python", 6), ("", 0)]
)
...
@pytest.mark.parametrize(
    "lista_numeros, resultado_esperado", [([1, 2, 3], 6), ([], 0), ([-1, -2, -3], -6)]
)
...
@pytest.mark.parametrize(
    "lista_numeros, resultado_esperado",
    [([1, 2, 3], 3), ([], None), ([-1, -2, -3], -1)],
)
...
@pytest.mark.parametrize(
    "string, prefixo, resultado_esperado",
    [
        ("Python is fun", "Python", True),
...
@pytest.mark.parametrize(
    "string, sufixo, resultado_esperado",
    [
        ("Python is fun", "fun", True),
...
@pytest.mark.parametrize(
    "string, caractere, resultado_esperado",
    [
...
@pytest.mark.parametrize(
    "string, caracteres_remover, resultado_esperado",
    [
        ("   Python   ", " ", "Python"),
        ("***Hello,world!***", "*", "Hello,world!"),
        ("###12345###", "#", "12345"),
...
@pytest.mark.parametrize(
    "string, substring_antiga, substring_nova, resultado_esperado",
    [
        ("Python is fun", "fun", "awesome", "Python is awesome"),
...
def test_replace_custom(string, substring_antiga, substring_nova, resultado_esperado):
    assert (
        replace_custom(string, substring_antiga, substring_nova) == resultado_esperado
    )